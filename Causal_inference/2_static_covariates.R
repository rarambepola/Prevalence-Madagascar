#script to run the PC algorithm with static covariates
...
static_index <- 29:37 #only use static variables

#run PC algorithm
ptm <- proc.time()
pc_both <- pcalg_both_steps(res_mat_list[static_index], alpha=alpha_use, 
                            last.index=length(static_index),
                            G_0=G_0[static_index, static_index],
                            alpha_step_2 = runif(1, 0.7, 0.9),
                            n_cl=4)
print(proc.time() - ptm)

save(list=c("pc_both"),
     file=paste0("Output/static_covariates_", seed, ".RData"))
